...
namespace Home.Devices.Unifi {
    ...
    public class UnifiApiClient : IDisposable {
        ...
        public async Task ConnectWebSocketAsync(bool isRetry = false) {
            ...
            try {
                ...
                if (isRetry) throw;
                if (await LoginAsync()) {
                    await ConnectWebSocketAsync(true);
                }
            }
        }
        ...
        public async Task<DevicesResponse> GetDevicesAsync(bool isRetry = false) {
            try {
                ...
                if (isRetry) throw;
                if (ex.StatusCode != HttpStatusCode.Unauthorized) throw;
                if (await LoginAsync()) {
                    return await GetDevicesAsync(true);
                }
                throw new Exception("Authentication error");
            }
        }

        public async Task<List<ClientModel>> GetClientsAsync(bool isRetry = false) {
            try {
                ...
                if (ex.StatusCode != HttpStatusCode.Unauthorized) throw;
                if (await LoginAsync()) {
                    return await GetClientsAsync(true);
                }
                throw new Exception("Authentication error");
            }
        }

        public async Task<bool> SetSwitchPortPowerEnabledAsync(string id, int port, bool enabled, bool isRetry = false) {
            try {
                ...
                if (ex.StatusCode != HttpStatusCode.Unauthorized) throw;
                if (await LoginAsync()) {
                    return await SetSwitchPortPowerEnabledAsync(id, port, enabled, true);
                }
                throw new Exception("Authentication error");
            }
        }
